...
def get_current_sprint_stats():
    """Get current sprint statistics for RSE team"""
    auth = HTTPBasicAuth(EMAIL, API_TOKEN)
    headers = {"Accept": "application/json"}
    ...
    rse_issues = [i for i in sprint_issues if is_rse_team(i['fields'].get(TEAM_FIELD, {}))]
    
    total_issues = len(rse_issues)
    in_progress = 0
    in_review = 0
    to_do = 0
    done = 0
...
def calculate_historical_metrics():
    """Calculate 12 months of historical metrics"""
    print("\n" + "="*60)
    print("COLLECTING 12-MONTH HISTORICAL DATA")
    print("="*60)
